...
const CAMMINO_EVENT_TYPE_META     = '_cammino_event_type';
const CAMMINO_EVENT_HIDE_IMAGE_META = '_cammino_event_hide_image';
const CAMMINO_PROJECT_HIDE_IMAGE_META = '_cammino_project_hide_image';
...
/** Return only the post types available when creating or changing a post. */
function cammino_get_selectable_post_placements(): array {
	return array_intersect_key( cammino_get_post_placements(), array( 'event' => true, 'project' => true ) );
}
...
function cammino_get_post_placement( int $post_id ): string {
	...
	return 'article' === $placement || isset( cammino_get_post_placements()[ $placement ] ) ? $placement : 'project';
}
...
function cammino_register_post_meta(): void {
	...
	register_post_meta( 'post', CAMMINO_EVENT_TYPE_META, array_merge( $common, array( 'type' => 'string' ) ) );
	register_post_meta( 'post', CAMMINO_EVENT_HIDE_IMAGE_META, array_merge( $common, array( 'type' => 'string' ) ) );
	register_post_meta( 'post', CAMMINO_PROJECT_HIDE_IMAGE_META, array_merge( $common, array( 'type' => 'string' ) ) );
...
function cammino_render_post_settings_meta_box( WP_Post $post ): void {
	...
	$placements = cammino_get_selectable_post_placements();
	...
	?>
	<?php if ( 'impact-story' === $placement ) : ?>
		<input id="cammino-post-placement" name="cammino_post_placement" type="hidden" value="impact-story">
		<p><strong><?php esc_html_e( 'Typ príspevku', 'cammino' ); ?></strong><br><?php esc_html_e( 'Existujúci príbeh pomoci', 'cammino' ); ?></p>
	<?php else : ?>
		<p>
	...
	<?php endif; ?>
	<p class="description">
		<?php esc_html_e( 'Podujatie alebo projekt. Obsah upravíte vo vizuálnom editore; zmena typu ho neprepíše.', 'cammino' ); ?>
	</p>
...
add_action( 'save_post_post', 'cammino_save_post_settings', 50 );
...
function cammino_save_post_settings( int $post_id ): void {
	...
	$placement = isset( $_POST['cammino_post_placement'] ) ? cammino_sanitize_post_placement( wp_unslash( $_POST['cammino_post_placement'] ) ) : cammino_get_post_placement( $post_id );
	$fields = array(
		CAMMINO_POST_PLACEMENT_META => $placement,
...
function cammino_update_visual_post_details( int $post_id, string $title, string $event_date = '', string $event_location = '', bool $hide_image = false, string $event_type = '', array $category_ids = array(), string $new_category = '' ): bool {
	...
	if ( 'event' === $placement ) {
		foreach ( array( CAMMINO_EVENT_DATE_META => $event_date, CAMMINO_EVENT_LOCATION_META => $event_location, CAMMINO_EVENT_TYPE_META => $event_type, CAMMINO_EVENT_HIDE_IMAGE_META => $hide_image ? '1' : '' ) as $meta_key => $value ) {
			if ( '' === $value ) {
				delete_post_meta( $post_id, $meta_key );
			} else {
				update_post_meta( $post_id, $meta_key, $value );
	...
	if ( 'project' === $placement ) {
		if ( $hide_image ) {
			update_post_meta( $post_id, CAMMINO_PROJECT_HIDE_IMAGE_META, '1' );
		} else {
			delete_post_meta( $post_id, CAMMINO_PROJECT_HIDE_IMAGE_META );
		}
	}
	$category_saved = true;
	if ( 'post' === get_post_type( $post_id ) ) {
	...
	return $title === (string) get_post_field( 'post_title', $post_id )
		...
		&& ( 'event' !== $placement || (
			...
			&& $event_type === (string) get_post_meta( $post_id, CAMMINO_EVENT_TYPE_META, true )
			&& ( $hide_image ? '1' : '' ) === (string) get_post_meta( $post_id, CAMMINO_EVENT_HIDE_IMAGE_META, true )
		) )
		&& ( 'project' !== $placement || ( $hide_image ? '1' : '' ) === (string) get_post_meta( $post_id, CAMMINO_PROJECT_HIDE_IMAGE_META, true ) );
}
...
/**
 * Return the optional event type used by directory labels and filters.
 *
 * @return array{slug:string,name:string,icon:string}
 */
function cammino_get_event_display_type( int $post_id ): array {
	$name = sanitize_text_field( (string) get_post_meta( $post_id, CAMMINO_EVENT_TYPE_META, true ) );
	$slug = '' !== $name ? sanitize_title( $name ) : '';
	if ( in_array( $slug, array( 'podujatie', 'podujatia', 'uncategorized' ), true ) ) {
		$name = '';
		$slug = '';
	}
...
function cammino_render_all_events( array $args = array(), int $page_id = 0 ): string {
	...
	$count_label = 1 === $event_count ? __( 'podujatie', 'cammino' ) : ( $event_count >= 2 && $event_count <= 4 ? __( 'podujatia', 'cammino' ) : __( 'podujatí', 'cammino' ) );
	...
	<div class="event-toolbar">
		...
		<div class="event-filters" role="group" aria-label="<?php esc_attr_e( 'Filtrovať podujatia podľa typu', 'cammino' ); ?>">
			<button class="event-filter is-active" type="button" data-event-filter="all" aria-pressed="true"><?php esc_html_e( 'Všetky', 'cammino' ); ?> <span><?php echo esc_html( (string) $event_count ); ?></span></button>
		...
		<p class="result-count" aria-live="polite"><strong data-visible-count><?php echo esc_html( (string) $event_count ); ?></strong> <span data-count-label><?php echo esc_html( $count_label ); ?></span></p>
	</div>
...
function cammino_render_all_projects( array $args = array(), int $page_id = 0 ): string {
	...
	<?php else : ?>
		<div class="project-category-browser">
			<div class="project-category-view" data-project-category-view>
			...
			<section class="project-results" id="project-results" aria-labelledby="project-results-title" hidden data-project-results>
				<header class="project-results__heading">
					<div><span><?php esc_html_e( 'Projekty v kategórii', 'cammino' ); ?></span><h2 id="project-results-title" data-project-selected-name></h2></div>
					<div class="project-results__tools">
						...
						<button class="project-back" type="button" data-project-back><i class="fa-solid fa-arrow-left-long" aria-hidden="true"></i> <?php esc_html_e( 'Späť na kategórie', 'cammino' ); ?></button>
					</div>
				</header>
				...
				$image_url    = '1' === (string) get_post_meta( $project_id, CAMMINO_PROJECT_HIDE_IMAGE_META, true )
					? false
					: get_the_post_thumbnail_url( $project_id, 'medium_large' );
				$description  = trim( (string) get_the_excerpt( $project ) );
 ...
 * start with an empty builder and six templates used by the inline editor.
 */
function cammino_render_post_visual_content( int $post_id ): string {
	...
	<template data-nstarter-content-template="gallery"><div class="article-gallery article-content-block" data-nstarter-content-item data-nstarter-content-type="gallery"><div class="article-gallery__item"><img src="<?php echo esc_url( $placeholder ); ?>" alt="" loading="lazy"></div></div></template>
...
function cammino_get_post_visual_content( int $post_id ): string {
	...
	$html = (string) preg_replace( '#<template\b[^>]*data-nstarter-content-template="important-link"[^>]*>.*?</template>#is', '', $html );
	$html = (string) preg_replace( '#<span\b[^>]*class="[^"]*article-impact-story__eyebrow[^"]*"[^>]*>.*?</span>#is', '', $html );
