document.addEventListener("DOMContentLoaded", () => {
  const causes = {
    ...
    general: {
      title: "Dar bez určenia",
      description: "Vašu pomoc nasmerujeme tam, kde je v danej chvíli najviac potrebná, a postaráme sa, aby skončila v správnych rukách.",
      icon: "fa-hand-holding-heart",
      theme: "plum",
      contentTitle: "Podpora tam, kde je najviac potrebná",
      contentLead: "Potreby programov a mladých ľudí sa menia. Neurčený dar nám umožňuje reagovať bez zbytočného čakania.",
      contentBody: "Prostriedky využijeme na aktuálnu prioritu — od pomoci konkrétnemu človeku až po zabezpečenie programu. O našej práci a výsledkoch komunikujeme otvorene a zrozumiteľne.",
      contentQuote: "Dôvera nám umožňuje reagovať práve tam, kde môže mať pomoc v tej chvíli najväčší význam.",
      contentCaption: "Neurčený dar drží pomoc pripravenú na situácie, ktoré nemožno naplánovať",
      secondaryTitle: "O použití rozhodujeme transparentne",
      secondaryBody: "Dar smerujeme podľa aktuálnych priorít, potrieb programov a situácie mladých ľudí. O výsledkoch a využití podpory pravidelne informujeme našu komunitu."
    }
  };

  const params = new URLSearchParams(window.location.search);
  const causeKey = causes[params.get("cause")] ? params.get("cause") : "education";
  ...
  document.querySelector("[data-cause-title]").textContent = cause.title;
  document.querySelector("[data-cause-description]").textContent = cause.description;
  document.querySelector("[data-content-title]").textContent = cause.contentTitle;
  document.querySelector("[data-content-lead]").textContent = cause.contentLead;
  document.querySelector("[data-content-body]").textContent = cause.contentBody;
  document.querySelector("[data-content-quote]").textContent = cause.contentQuote;
  document.querySelector("[data-content-caption]").textContent = cause.contentCaption;
  document.querySelector("[data-content-secondary-title]").textContent = cause.secondaryTitle;
  document.querySelector("[data-content-secondary-body]").textContent = cause.secondaryBody;
  ...
  document.querySelector("[data-content-image]").alt = `${cause.title} — ilustračná fotografia programu`;
  ...
  revealItems.forEach((item) => item.style.setProperty("--detail-delay", `${item.dataset.delay || 0}ms`));
  ...
  const updateSummary = () => {
    summaryAmount.textContent = `${selectedAmount || 0} €`;
    summaryFrequency.textContent = selectedFrequency === "monthly" ? "Mesačný dar" : "Jednorazový dar";
  };
  ...
  document.querySelectorAll(".newsletter").forEach((form) => form.addEventListener("submit", (event) => event.preventDefault()));
