...
home_v2_expect( str_contains( $styles, '.home-v2 .event-image::after {' ) && str_contains( $styles, 'background: rgba(33, 26, 51, 0.08);' ), 'The inherited event image gradient is replaced with a solid tint.' );
home_v2_expect( str_contains( $styles, '.site-nav > a:not(.button, .language-link)::after' ) && str_contains( $styles, 'background-color: var(--v2-coral);' ), 'Header link underlines use a solid accent color.' );
home_v2_expect( str_contains( $styles, '.home-v2 .hero-copy h1 em {' ) && str_contains( $styles, 'color: var(--v2-plum);' ), 'Emphasized heading text receives an accent text color.' );
