...
home_v2_expect( str_contains( $styles, '--v2-spectrum: linear-gradient(110deg, var(--v2-spectrum-light) 0%, var(--v2-spectrum-light) 13%, var(--v2-spectrum-peach) 33%, var(--v2-spectrum-violet) 49%, var(--v2-spectrum-blue) 69%, var(--v2-spectrum-blue) 100%);' ), 'The core background follows the requested 25 / 20 / 20 / 45 visual weighting.' );
home_v2_expect( str_contains( $styles, 'Never crop the spectrum on large surfaces' ) && str_contains( $styles, 'background-size: 100% 100%;' ), 'Large surfaces always show the complete four-color gradient.' );
home_v2_expect( str_contains( $styles, 'Modern art direction' ) && str_contains( $styles, 'background-image: var(--v2-spectrum);' ), 'One four-color gradient is composed on the body canvas.' );
home_v2_expect( str_contains( $styles, '.home-v2 .hero,' ) && str_contains( $styles, 'background-color: transparent;' ), 'Selected sections reveal the continuous body background.' );
home_v2_expect( str_contains( $styles, '.home-v2 .story-section {' ) && str_contains( $styles, 'background-color: #a28ebf;' ), 'Solid palette sections use focused lighting rather than repeated section gradients.' );
...
home_v2_expect( str_contains( $styles, '.home-v2 .event-list {' ) && str_contains( $styles, 'border-color: transparent;' ) && str_contains( $styles, 'background: transparent;' ) && str_contains( $styles, 'box-shadow: none;' ), 'The event list container has no background, border, or shadow.' );
home_v2_expect( str_contains( $styles, 'box-shadow: -34px 34px 86px rgba(239, 199, 202, 0.62), 36px -30px 96px rgba(88, 152, 204, 0.34);' ), 'Blurred palette shadows create atmospheric color around solid elements.' );
