...
home_v2_expect( str_contains( $theme, "define( 'CAMMINO_DONATE_URL', 'https://cammino.darujme.sk/darujmeusmev/' )" ) && str_contains( $theme, 'class="nav-donate"' ) && str_contains( $theme, 'esc_url( CAMMINO_DONATE_URL )' ), 'The header donation button is independent of the WordPress menu and uses the fixed Darujme URL.' );
home_v2_expect( str_contains( $theme, 'cammino_render_shared_menu( true )' ) && str_contains( $theme, 'exclude_donation_link' ), 'The header suppresses legacy donation entries from the WordPress menu to avoid a duplicate CTA.' );
home_v2_expect( ! str_contains( $styles, '.site-nav > a:last-child' ), 'Header CTA styling targets the dedicated donation button instead of the final menu item.' );
