...
$before_restore = nstarter_get_snapshot_html( 1 );
$before_restore_token = nstarter_snapshot_token( 1 );
$response = history_call( 'nstarter_ajax_restore_snapshot_version', array( 'version_id' => $restored['id'] ) );
history_expect( $response->status === 200 && $response->data['versionId'] === $restored['id'], 'Restore selects the requested version.' );
history_expect( nstarter_get_snapshot_html( 1 ) === $before_restore && nstarter_get_snapshot_history( 1 ) === $state, 'Restore leaves saved HTML and the entire history unchanged.' );
history_expect( $response->data['snapshotToken'] === $before_restore_token && nstarter_snapshot_token( 1 ) === $before_restore_token, 'Restore keeps the original concurrency token.' );
$response = history_call( 'nstarter_ajax_save_snapshot', array( 'html' => wp_slash( $restored['html'] ) ) );
history_expect( $response->status === 200 && nstarter_get_snapshot_html( 1 ) === $restored['html'], 'Only pressing Save commits restored content.' );
history_expect( nstarter_get_snapshot_history( 1 )['previous'][0]['html'] === $before_restore, 'Saving the restored draft archives the replaced content.' );
...
$before_restore_state = nstarter_get_snapshot_history( 1 );
$response = history_call( 'nstarter_ajax_restore_snapshot_version', array( 'version_id' => $old_version['id'] ) );
$merged = nstarter_render_merged_page_html( 1, $old_version['html'] );
...
history_expect( nstarter_get_snapshot_history( 1 ) === $before_restore_state && ! empty( $merged['conflicts'] ), 'Loading a restored draft reports unmatched content without changing history.' );
$before = nstarter_get_snapshot_html( 1 );
